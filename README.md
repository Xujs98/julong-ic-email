...
新地址生成后处于“待首次使用”状态，不消耗有效期。第一次打开页面或请求同 token 的 `/data` 接口时，系统按当时设置的秒数写入激活时间和精确到秒的过期时间；重复访问不会续期。该页面无需登录，只能查看 token 对应的单个邮箱。邮箱 API 地址继续使用原有 `mailbox_key`，没有时间过期规则。

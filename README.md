...
- 出库与发货：勾选库存邮箱后点击“出库”，邮箱状态会改为“出库”并移入侧边栏“取件码与发货”；出库列表继续支持邮件查询、验证码和 HTML 地址，也可批量退回库存。

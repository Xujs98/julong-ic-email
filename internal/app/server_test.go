...
func TestSystemSettingsTemplateIncludesMailRetentionAndExpiryDeleteSwitches(t *testing.T) {
	...
	for _, want := range []string{
		...
		`id="systemSettingsSaveButton"`,
		`id="systemSettingsSaveButtonLabel"`,
		`type="button" onclick="saveSystemSettings()"`,
		...
		`applySystemSettings`,
		`let systemSettingsSaving = false`,
		`button.setAttribute('aria-busy', 'true')`,
		`setSystemSettingsStatus(` + "`" + `保存失败：${message}` + "`" + `, 'pending')`,

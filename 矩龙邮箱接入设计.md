...
> 目标：把截图里的 矩龙邮箱 / 隐私邮箱 API 工作台接入当前 Go 面板，作为新的邮箱来源使用。注册、登录拿 CPA、登录设置 2FA/TOTP 的 OpenAI/Codex 协议流程不改，只替换“邮箱取号 + 轮询验证码”这一层。
...
4. `icloud_api`：矩龙邮箱 API 取码平台。

推荐第一阶段只做 **接入外部 矩龙邮箱 API 地址**，不在当前项目里直接实现 Apple/iCloud 登录、隐私邮箱创建、会话维护和邮件同步。原因：
...
| 名称 | 含义 |
| --- | --- |
| 矩龙邮箱 | 隐私邮箱平台，用 iCloud 账号创建/管理 Hide My Email 地址，并对外提供取码 API |
...
| `icloud_api_base_url` | string | 空 | 矩龙邮箱 平台地址 |
...
13. `导出邮箱API` 和 `只导出邮箱` 支持 `txt/csv/tsv/jsonl`，并支持按 `account_id` 过滤。
14. 管理员数据管理页支持按平台用户 `owner_id` 和 Apple 登录态 `account_id` 组合过滤导出；普通用户不会因为传参越权。
15. iCloud `accountLogin` 后续步骤和 Hide My Email 列表同步遇到 EOF/timeout 等临时网络错误时会短重试；2FA/OTP 校验不会重复提交。
16. 每个隐私邮箱的 API token 固定保存在 `data_path` 的 `mailboxes[].api_token`，服务重启、重新构建和前端更新不会改变 token。
...
| 字段 | 所属对象 | 用途 |
| --- | --- | --- |
| `owner_id` | 用户、登录态、邮箱、邮件 | 平台账号权限隔离；普通用户只能看自己的数据 |
...
部署或迁移时必须先备份服务器 `state.json`。合并时：

1. 以 `email` 匹配邮箱。
2. 已存在邮箱优先保留服务器原 `api_token`。
3. 只合并标签、状态、`owner_id`、`account_id`、同步时间、邮件计数和新增邮件。
4. 新邮箱才使用来源文件里的 `api_token`。
5. 合并后恢复服务用户属主和 `0600` 文件权限。

如果旧 API 地址突然不可用，优先检查是否发生过状态文件覆盖；可从部署前备份里按邮箱找回旧 `api_token`。
## 域名邮箱扩展（2026.08.19）

- 新增 `Domain` 资产，按平台用户隔离；域名可启用、停用与删除。存在关联域名邮箱时拒绝删除域名，避免收件路由与存量数据失配。
- 新增 `Mailbox.provider`（`icloud` / `domain`）和 `domain_id`。域名邮箱复用既有邮箱 API、HTML 接码链接、消息存储与权限模型。
- 内置 SMTP 服务只接受已启用域名下、已生成且启用的精确收件地址；DATA 内容解析纯文本和 HTML MIME 分支，并以 `domain_smtp` 来源写入现有消息表。
- 域名邮箱取码优先且只读取本地 SMTP 已入库邮件，不触发 iCloud IMAP 同步；删除域名邮箱只删除本地邮箱、邮件、API 与 HTML 链接。
- `domain_smtp_enabled`、`domain_smtp_host`、`domain_smtp_port` 和 `domain_smtp_max_message_bytes` 控制运行时服务；Docker 默认暴露容器 2525，生产 MX 可由宿主机 TCP 25 映射。
- `domain_smtp_cert_file` 与 `domain_smtp_key_file` 为可选的 `STARTTLS` 证书配置；Compose 默认从 `/app/tls` 只读挂载证书目录，状态接口通过 `tls_enabled` 返回是否已加载证书。
